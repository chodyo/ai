...
class PotentialField(object):
	...
	def get_vector(self, coord):
		...
		print coord.x
		print coord.y
		x, y = self.get_attractive(self.goal, coord, self.attractive_strength)
		ret_x += x
		ret_y += y
		print "attract",ret_x, ret_y
	...
	def get_attractive(self, obj, coord, strength):
		...
		g = obj
		r = float(g.r)
		s = float(self.worldsize)/16				# this value needs to be adjusted based on experimentation

		print "codytest: ", coord.x
		d = math.sqrt((g.x-coord.x)**2+(g.y-coord.y)**2)
	...
	def get_repulsive(self, obj, coord, strength):
		...
		d = math.sqrt((o.x-coord.x)**2+(o.y-coord.y)**2)
